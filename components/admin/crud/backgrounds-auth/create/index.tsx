...
const BackgroundsAuthCreate = () => {
  const [background, setBackground] = useState<BackgroundsAuth>({
    url: '',
    order: 0,
  });
  ...
  const handleSubmit = async (e: FormEvent) => {
    try {
      e.preventDefault();
      await backgrounds_auth_service.create(background);
...
export default BackgroundsAuthCreate;
